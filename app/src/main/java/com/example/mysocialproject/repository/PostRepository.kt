...
class PostRepository {
            ...
            try {
                fireStore.runTransaction { transaction ->
                    ...
                    if (likeQuerySnapshot.isEmpty) {
                        // Tạo bản ghi "reactions" mới nếu chưa tồn tại
                        val postDocument =
                        ...
                        transaction.set(fireStore.collection("reactions").document(), newReaction)
                    } else {
                        // Cập nhật bản ghi "reactions" hiện có
                        val likeDocumentSnapshot = likeQuerySnapshot.documents[0]
                        val existingReaction = likeDocumentSnapshot.toObject(Reaction::class.java)

                        if (existingReaction != null) {
                            val updatedReactions = if (existingReaction.emoji.size >= 4) {
                                val updatedList = existingReaction.emoji.toMutableList()
                                updateReactions(updatedList, icon)
                                updatedList
                            } else {
                                existingReaction.emoji.toMutableList().apply { add(icon) }
                            }
    ...
    fun getlikepost(postId: String, listinfolike: (List<Pair<String, List<String>>>) -> Unit) {
        val likesRef = fireStore.collection("reactions").whereEqualTo("postId", postId)
        ...
        likesRef.addSnapshotListener { querySnapshot, error ->
            ...
            querySnapshot?.documents?.forEach { likeDoc ->
                val reaction = likeDoc.toObject(Reaction::class.java)
                reaction?.let {
                    ...
                    fireStore.collection("users").document(userId)
                        ...
                        .addOnSuccessListener { userSnapshot ->
                            ...
                            Log.e(
                                "PostRepository",
                                "Lỗi khi lấy reactions: ${exception.message}",
                                exception
                            )
                        }
                }
    ...
    suspend fun updateViewByUser(postId: String): Boolean {
        val postRef = fireStore.collection("posts").document(postId)
        val currentId = auth.currentUser?.uid ?: return false // Handle no user logged in

        return try {
            postRef.update("viewByUser", FieldValue.arrayUnion(currentId)).await()
            Log.d("PostRepository", "Post $postId viewByUser $currentId")
            true
        } catch (e: FirebaseFirestoreException) {
            if (e.code == FirebaseFirestoreException.Code.NOT_FOUND) {
                Log.e("PostRepository", "Document $postId not found", e)
            } else {
                Log.e("PostRepository", "Error updating viewByUser for $postId", e)
            }
            false
        }
    }
            ...
            postListenerRegistration = postRef.addSnapshotListener { snapshot, error ->
                ...
                if (snapshot != null) {
                    ...
                    val newPosts = posts.filter { post ->
                        !post.viewByUser.contains(currentUserId)
                    }
